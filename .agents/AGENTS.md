...
7. **JUCE Discrete Parameters**: NEVER use `AudioParameterChoice` or `AudioParameterInt` for parameters representing discrete menus/choices (e.g., filter types). ALWAYS use a continuous `AudioParameterFloat` with `NormalisableRange<float>(0.0f, max_val, 1.0f)`. If changing a parameter's data type, ALWAYS rename its `ParameterID` string to sever its link to poisoned VST3 DAW caches. If a parameter is strictly internal logic (like UI state), ALWAYS pass `juce::AudioParameterFloatAttributes().withAutomatable(false)`.
8. **Exhaustive Dependency Checking**: Whenever changing the ID, type, or bounds of a variable/parameter in one file (e.g., C++), ALWAYS perform an exhaustive global search using `grep_search` across all related environments (HTML, JS, CSS, other C++ headers) to guarantee no references or UI bindings are broken. Never assume a parameter is only used in one place.
